...
class GostCrypt(object):
    ...
    def encrypt(self, plain_msg):
        ...
        enc_msg_list = []
        msg_list = [bin(plain_msg)[i:i + 64] for i in range(0, len(bin(plain_msg)), 64)]
        for msg in msg_list:
            ...
            enc_msg_list.append( (left_part << 32) | right_part) #сливаем половинки вместе
        enc_msg = ''
        for enc in enc_msg_list:
            enc_msg += str(enc)
        return int(enc_msg)
    ...
    def decrypt(self, crypted_msg):
        ...
        dec_msg_list = []
        msg_list = [int(bin(crypted_msg)[i:i + 64], 2) for i in range(0, len(bin(crypted_msg)), 64)]
        for msg in msg_list:
            assert self._bit_length(msg) <= 64
            left_part = msg >> 32
            right_part = msg & 0xFFFFFFFF
            ...
            for i in range(24):
                ...
                dec_msg_list.append( (left_part << 32) | right_part )#сливаем половинки вместе
        dec_msg = ''
        for dec in dec_msg_list:
            dec_msg += str(dec)
        return int(dec_msg)
